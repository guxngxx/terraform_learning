...
locals {
  ...
  common_tags = {
    ...
    Owner     = lower("testing123")
    App       = lower("testing123")
    Service   = lower("testing123")
    AppTeam   = lower("testing123")
    CreatedBy = lower("testing123")
  }
}
...
#   for_each          = var.private_subnets
#   vpc_id            = aws_vpc.vpc.id
#   cidr_block        = cidrsubnet("10.0.0.0/16", 8, each.value + 100)
#   availability_zone = tolist("data.aws_availability_zones.available.names")[each.value]
...
// without using dynamic block to create 2 security group rules
...
// with dynamic block + local variables
locals {
  ingress_rules = [
    {
      port        = 443
...
// with dynamic block + input variables
